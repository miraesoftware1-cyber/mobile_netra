...
| `SCH` | 일정관리 | SCH_01~02 | |
| `APVMNG` | 승인 관리 | APVMNG_01~02 | STATIC (ERP 권한 불필요) |

## STATIC 메뉴

ERP `ENV_MOBILE_PERMIS`에 등록되지 않아도 항상 표시되는 모바일 네이티브 메뉴입니다.  
`src/app/(main)/menu/page.tsx`의 `STATIC_SECTIONS`에 정의되어 있으며, DB 메뉴 필터링 시 제외됩니다.
