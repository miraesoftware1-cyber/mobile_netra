...
BEGIN TRY
    ...
    BEGIN
        SELECT
            ...
            s.STEP_ID,
            s.STEP_NO,
            ISNULL(s.STEP_TYPE, '')     AS STEP_TYPE,
            s.PUSH_YN,
...
BEGIN TRY
    INSERT INTO TB_MOBILE_APVMNG_STEP_APV (REQ_ID, STEP_NO, APV_TYPE, EMP_CODE)
    VALUES (@REQ_ID, @STEP_NO, @APV_TYPE, @EMP_CODE)
...
BEGIN TRY
    SELECT sa.EMP_CODE,
           ISNULL(e.emp_name, sa.EMP_CODE) AS EMP_NAME
    FROM TB_MOBILE_APVMNG_STEP_APV sa WITH(NOLOCK)
    LEFT JOIN mst_emp e WITH(NOLOCK) ON e.emp_code = sa.EMP_CODE
    WHERE sa.REQ_ID = @REQ_ID AND sa.STEP_NO = @STEP_NO
END TRY
BEGIN CATCH
    select -1 as Flag, ERROR_MESSAGE() as MSG;
END CATCH
